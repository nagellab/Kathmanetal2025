...
use_flies = [21:26, 30, 32:35, 37:39]; sdm = .5; line_label = 'hAck';
% % use_flies = 35;
% % use_flies = [25]; sdm = .5; line_label = 'hAck';
% % use_flies = [41:51]; line_label = 'FC1';
% use_flies = [81, 83:86]; line_label = 'hAck_split_nick'; 
% use_flies = [81, 84:87]; line_label = 'hAck_split_nick'; 
% % use_flies = [66:68, 71:74]; line_label = 'hAck_split1'; 
% % use_flies = 151:154; line_label = 'hAck_split_aaron'; 

print_figs = 0;
add2txt = '_bmplngth3_longavelwin';
plot_trials = 0;
bmp_p1 = 3;
bmp_p2 = 3; 
...
%% ---------------- NEW: per-fly bump/trial counters ----------------
bumps_per_fly = zeros(length(use_flies), 1);            % included bumps per fly
trials_with_bumps_per_fly = zeros(length(use_flies), 1);% trials containing >=1 included bump (per fly)
bump_trials_mask = cell(length(use_flies), 1);          % stores trial indices (in data) that had >=1 included bump
total_bumps_per_fly = zeros(length(use_flies),1);   % ALL detected bumps (before inclusion gate)
% -------------------------------------------------------------------
...
%                 allamp_thr = [allamp_thr; data(t).bump_amp(~isnan(data(t).bump_amp(:,1)))];
    ...
    for t = use_trials
        ...
        % ---------------- NEW: trial had any included bump? ----------------
        this_trial_had_included_bump = false;
        % ------------------------------------------------------------------
        ...
        oni = []; offi = []; 
%         [bump, oni, offi] = bumpfinder2(dff, tt, 5, 5, base_thr);
        [bump, oni, offi] = bumpfinder2(dff, tt, bmp_p1, bmp_p2, base_thr);
...
%                 if oni(c) > odorevent(1)-10 && offi(c) > odorevent(2)
            if oni(c) >= odorevent(1) && oni(c) <= odorevent(2) && offi(c) > odorevent(2)

                    % ---------------- NEW: count bumps/trials per fly ----------------
                    if m == 1
                        bumps_per_fly(f) = bumps_per_fly(f) + 1;
                        this_trial_had_included_bump = true;
                        n = n + 1;
                    end
                    % ----------------------------------------------------------------

                    win1 = (offi(c)+win(1)*fps):(offi(c)+win(2)*fps);
                    win1 = win1(win1 < length(tt) & win1 > 0);
                    wint1 = tt(win1)-tt(offi(c));
                    ...
                    if length(win1) ~= diff(win)*fps+1
                        ...
                        if over
                            this_off(end-length(this_met)+1:end) = this_met;
                        else
                            this_off(1:length(this_met)) = this_met;
                        end
                        offs{m} = [offs{m}, this_off];
                    else
                        offt = wint1;
                        offs{m} = [offs{m}, met(win1)];
                    end
        ...
        % ---------------- NEW: after finishing this trial, log trial count ----------------
        if this_trial_had_included_bump
            trials_with_bumps_per_fly(f) = trials_with_bumps_per_fly(f) + 1;
            bump_trials_mask{f} = [bump_trials_mask{f}, t];
        end
        % -------------------------------------------------------------------------------
    end
...
%% ---------------- NEW: print per-fly bump/trial summary ----------------
fly_id = use_flies(:);
% T_bumps = table(fly_id, trials_with_bumps_per_fly, bumps_per_fly, ...
%     'VariableNames', {'fly','nTrialsWithIncludedBumps','nIncludedBumps'});
T_bumps = table( ...
    fly_id, ...
    trials_with_bumps_per_fly, ...
    bumps_per_fly, ...
    total_bumps_per_fly, ...
    'VariableNames', { ...
        'fly', ...
        'nTrialsWithIncludedBumps', ...
        'nIncludedBumps', ...
        'nTotalDetectedBumps'});

disp(T_bumps)

% Optional quick totals sanity check:
fprintf('Total included bumps (sum across flies) = %d (script n = %d)\n', sum(bumps_per_fly), n);
fprintf('Flies with >=1 included bump = %d (script N = %d)\n', sum(bumps_per_fly > 0), N);
% ----------------------------------------------------------------------
...
for m = 1:nm
    if ~isempty(allfly_mnoffs{m})
        ...
        % --- your sliding window stats code unchanged ---
        fs = 100;
        win_size_sec = 0.5;
        step_size_sec = 0.1;
        ...
        for i = 1:n_windows
            ...
            time_centers(i) = (start_idx + end_idx) / 2 / fs;
        end

        [h_fdr, crit_p, adj_p] = fdr_bh(p_values);
...
figure(1);
if print_figs
%     print_fig(1, gcf, [subpath 'bumpofftrigVon_pulse_' line_label add2txt], 'eps', 1); 
%     exportgraphics(gcf, [subpath 'bumpofftrigVon_pulse_' line_label add2txt, '.png']);
    savefig([subpath 'bumpofftrigVon_pulse_' line_label add2txt])
end

figure(2);
if print_figs
%     print_fig(1, gcf, [subpath 'bumpofftrigVonStats_pulse_' line_label add2txt], 'eps', 1); 
%     exportgraphics(gcf, [subpath 'bumpofftrigVonStats_pulse_' line_label add2txt, '.png'])
    savefig([subpath 'bumpofftrigVonStats_pulse_' line_label add2txt])
end
